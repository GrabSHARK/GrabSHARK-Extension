...
export const ACTION_DEFINITIONS: Record<CaptureActionType, { icon: string; label: string }> = {
  ...
  highlight: {
    // bi-highlighter (from Main Toolbox)
  ...
  erase: {
    icon: `<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-eraser" viewBox="0 0 16 16">
  <path d="M8.086 2.207a2 2 0 0 1 2.828 0l3.879 3.879a2 2 0 0 1 0 2.828l-5.5 5.5A2 2 0 0 1 7.879 15H5.12a2 2 0 0 1-1.414-.586l-2.5-2.5a2 2 0 0 1 0-2.828l6.879-6.879zm2.121.707a1 1 0 0 0-1.414 0L4.16 7.547l5.293 5.293 4.633-4.633a1 1 0 0 0 0-1.414l-3.879-3.879zM8.746 13.547 3.453 8.254 1.914 9.793a1 1 0 0 0 0 1.414l2.5 2.5a1 1 0 0 0 .707.293H7.88a1 1 0 0 0 .707-.293l.16-.16z"/>
</svg>`,
    label: 'smartActions.erase',
  },
  clip: {
    // bi-crop (from Main Toolbox "camera" icon)
  ...
  add_note: {
    // bi-chat-square-dots (from Main Toolbox)
...
export class CaptureActionBarRenderer {
  /**
   * Render the action bar content (formerly renderActionMode)
   */
  render(
    ...
    // Preserve animation state classes (hidden, dark/light) during render
    const isHidden = container.classList.contains('ext-lw-capture-actionbar-hidden');

    container.className = 'ext-lw-capture-actionbar-outer';
    if (isHidden) container.classList.add('ext-lw-capture-actionbar-hidden');
    if (isDark) {
      container.classList.add('ext-lw-dark');
      container.classList.remove('ext-lw-light');
    } else {
      container.classList.add('ext-lw-light');
      container.classList.remove('ext-lw-dark');
    }
    ...
    if (ContentExtractor.hasTextContent(target) || (!ContentExtractor.hasLink(target) && !ContentExtractor.hasImage(target) && !ContentExtractor.hasVideo(target) && !ContentExtractor.hasFile(target))) {
    ...
    // Get all extracted content using ContentExtractor
    ...
    const html = actions.map((action) => {
      ...
      if (action === 'save_link' && links.length > 1) {
        return this.renderDropdownButton('save_link', i18n.t('smartActions.saveLink'), linksWithLabels.length,
          linksWithLabels);
      }

      // Special handling for save_image with multiple images
      if (action === 'save_image' && images.length > 1) {
        return this.renderDropdownButton('save_image', i18n.t('smartActions.saveImage'), images.length,
          images.map(url => ({ url, label: this.truncateUrl(url) })));
      }

      // Special handling for save_video with multiple videos
      if (action === 'save_video' && videos.length > 1) {
        return this.renderDropdownButton('save_video', i18n.t('smartActions.saveVideo'), videos.length,
          videos.map(url => ({ url, label: this.truncateUrl(url) })));
      }

      // Special handling for save_file with multiple files
      if (action === 'save_file' && files.length > 1) {
        return this.renderDropdownButton('save_file', i18n.t('smartActions.saveFile'), files.length,
    ...
    // Header with favicon, Smart Actions title (left) and close button (right)
    const headerBar = `
      <div class="ext-lw-capture-actionbar-header">
        <div class="ext-lw-capture-actionbar-title-container">
            <img src="${chrome.runtime.getURL('16.png')}" width="18" height="18" alt="" class="ext-lw-capture-actionbar-icon" />
    ...
    container.innerHTML = `
        <div class="ext-lw-capture-actionbar-inner">
            ${headerBar}
            <div class="ext-lw-capture-actionbar-actions">${html}</div>
        </div>
  ...
  private renderDropdownButton(
    ...
    return `
        <div class="ext-lw-capture-save-link-container" data-dropdown-type="${action}">
          ...
          <div class="ext-lw-capture-link-dropdown">
            <div class="ext-lw-capture-link-dropdown-scroll">
              ${items.map((item) => `
                <button class="ext-lw-capture-link-item" data-url="${this.escapeHtml(item.url)}" data-action-type="${action}" title="${this.escapeHtml(item.url)}">
                  ${def.icon}
                  <span class="ext-lw-capture-link-url">${this.escapeHtml(item.label)}</span>
                </button>
              `).join('')}
            </div>
