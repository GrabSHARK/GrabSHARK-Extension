...
export const DeleteDialog: FC<DeleteDialogProps> = ({ isOpen, onClose, onDelete }) => {
    ...
    return (
        <div className="absolute bottom-4 left-4 right-4 z-50 flex flex-col gap-3 p-4 bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 shadow-2xl rounded-2xl animate-in slide-in-from-bottom-5 fade-in duration-200">
            <div className="flex flex-col gap-1">
                <h4 className="text-sm font-semibold text-zinc-900 dark:text-zinc-100">{t('editLink.deleteLink') || "Delete Link?"}</h4>
                <p className="text-xs text-zinc-500 dark:text-zinc-400">{t('editLink.deleteConfirm') || "This action cannot be undone."}</p>
            </div>
